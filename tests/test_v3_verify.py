...
renderer = RendererAgent(output_dir="artifacts/test_v3")
nodes = [
...
renderer._generate_graph_html(graph, renderer.output_dir / "test_v3.html")

with open(renderer.output_dir / "test_v3.html", 'r', encoding='utf-8') as f:
    html = f.read()
...
checks = {
    ...
    'no diamond shape': "shape:'diamond'" not in html,
    'merge size:8': 'size:8' in html,
    'mergeLevel=targetLevel-0.5': 'targetLevel-0.5' in html,
    'chosen:false': 'chosen:false' in html,
    'no setData': 'setData' not in html,
    'physics disabled': 'physics.enabled=false' in html or 'physics:{enabled:false}' in html,
...
with sync_playwright() as p:
    ...
    page.goto(f"file:///{os.path.abspath('artifacts/test_v3/test_v3.html').replace(os.sep, '/')}")
    ...
    # Test 3: Merge nodes are small dots
    ...
    # Test 4: No color change on hover (hover disabled)
    results['hover_disabled'] = 'hover:false' in html

    # Test 5: Physics disabled
    results['physics_disabled'] = page.evaluate("typeof physicsEnabled !== 'undefined' ? !physicsEnabled : false")

    # Test 6: Merge nodes at half-levels (not integer levels)
    if merge_nodes:
    ...
    # Test 7: Each target has exactly one merge node
    target_ids = set()
    for e in page.evaluate("rawEdges"):
        if e.get('type') == 'derives_from':
            target_ids.add(e.get('from'))
    results['one_merge_per_target'] = len(merge_nodes) == len(target_ids)

    # Test 8: Edges from merge to target exist
    ...
    # Test 10: Click merge node shows derivation
    if merge_nodes:
